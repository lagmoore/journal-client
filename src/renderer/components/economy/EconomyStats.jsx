...
const EconomyStats = ({ data, year }) => {
  ...
  const calculateDifference = () => {
    ...
    return ((data.actualIncome - data.predictedIncome) / data.predictedIncome) * 100;
  };
      ...
      <div className="bg-base-100 rounded-lg shadow-card border border-base-300 p-6">
        ...
        <div className="mt-4">
          <div className="bg-base-200 rounded-full h-2 overflow-hidden">
            <div 
              className={`h-full rounded-full ${isBudgetMet ? 'bg-success' : 'bg-warning'}`}
              style={{ width: `${Math.min(100, (data.actualIncome / data.budget) * 100)}%` }}
            ></div>
          </div>
          <div className="mt-2 text-sm text-neutral">
            {data.budget > 0 
              ? `${Math.round((data.actualIncome / data.budget) * 100)}% ${t('economy.stats.ofBudget')}`
      ...
      <div className="bg-base-100 rounded-lg shadow-card border border-base-300 p-6">
        <div className="flex items-start justify-between">
          ...
          <div className="bg-info bg-opacity-10 p-3 rounded-full">
            <svg className="w-6 h-6 text-info" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className={`mt-4 flex items-center ${getDifferenceColor()}`}>
            ...
            <span className="text-sm">
              {difference === 0 
                ...
                : `${Math.abs(difference).toFixed(1)}% ${difference > 0 
