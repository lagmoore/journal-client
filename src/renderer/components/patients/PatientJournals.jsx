...
const PatientJournals = ({ journals, medications, patientId, onCreateJournal, onCreateMedication }) => {
  ...
  const groupEntriesByDate = (entries) => {
    ...
    Object.keys(groups).forEach(date => {
      groups[date].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    });
  ...
  const filteredJournals = journals.filter(journal => {
    ...
    if (searchTerm) {
      ...
      if (
        ...
        !(journal.medicationName && journal.medicationName.toLowerCase().includes(lowerSearchTerm))
      ) {
        return false;
      }
    }
    ...
    if (filters.entryType !== 'all' && journal.entryType !== filters.entryType) {
      return false;
    }
    ...
    switch (entry.entryType) {
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {entry.medicationTime && (
                <div className="text-sm text-neutral">
                  {new Date(entry.medicationTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </div>
              )}
            </div>
            {entry.content && <div className="mt-2 text-sm">{entry.content}</div>}
          </div>
        );
        ...
        return (
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getTestResultBadgeClass(entry.testResult)}`}>
                {entry.testResult === 'positive' ? t('common.positive') : t('common.negative')}
              </span>
            </div>
            {entry.testResult === 'positive' && entry.positiveSubstances?.length > 0 && (
              <div className="mt-1">
                <span className="text-sm font-medium">{t('journals.drugTest.positiveSubstances')}:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {entry.positiveSubstances.map((substance, index) => (
                    <span key={index} className="px-2 py-0.5 text-xs bg-base-200 rounded-full">
                      {substance}
                    </span>
                  ))}
                </div>
      ...
      case 'incident':
        return (
          <div>
            <div className="flex items-center gap-2 mb-1">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getIncidentSeverityBadgeClass(entry.incidentSeverity)}`}>
                {t(`journals.incident.severity.${entry.incidentSeverity || 'low'}`)}
              </span>
            </div>
  ...
  const hasActiveMedications = medications && medications.some(med => med.isActive);
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        ...
        >
          ...
          <option value="medication">{t('journals.entryTypes.medication')}</option>
      ...
      {groupedJournals.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="absolute -left-9 mt-1 w-4 h-4 rounded-full bg-base-100 border-2 border-base-300 flex items-center justify-center">
                      <EntryTypeIcon 
                        ...
                        className={`h-3 w-3 ${
                          entry.entryType === 'incident' ? 'text-error' : 
                          entry.entryType === 'medication' ? 'text-success' : 
                          entry.entryType === 'drug_test' ? 'text-primary' : 
                          'text-neutral'
                        }`} 
                      />
                    </div>
        ...
        <div className="text-center py-8">
          <p className="text-neutral text-lg">
            {searchTerm || filters.entryType !== 'all' || filters.status !== 'all' || filters.period !== 'all'
