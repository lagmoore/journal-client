...
const PatientMedications = ({ medications, patientId, onCreateMedication, onUpdateMedication, onDeleteMedication }) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">{t('patients.medications.title')}</h2>
        <button
          onClick={() => {
            setEditingMedication(null);
            setShowForm(true);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormInput
                  ...
                  placeholder={t('patients.medications.standardDosePlaceholder')}
                  required
              ...
              <div className="mb-4">
                ...
                {formErrors.frequency && (
                  <p className="text-error text-sm mt-1">{formErrors.frequency}</p>
                )}
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder={t('patients.medications.instructionsPlaceholder')}
      ...
      {activeMedications.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-medium mb-3">{t('patients.medications.active')}</h3>
          <div className="bg-base-100 rounded-lg shadow-sm overflow-x-auto">
            <table className="min-w-full divide-y divide-base-300">
              <thead className="bg-base-200">
                <tr>
                  <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-neutral uppercase tracking-wider">
                  ...
                  <th scope="col" className="px-4 py-3 text-right text-xs font-medium text-neutral uppercase tracking-wider">
                    {t('common.actions')}
                  </th>
                </tr>
              </thead>
                ...
                {activeMedications.map(med => (
                  <tr key={med.id} className="hover:bg-base-200">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      <div>{formatDate(med.startDate)} {med.endDate && `- ${formatDate(med.endDate)}`}</div>
                      {med.instructions && (
                        <div className="text-neutral text-xs mt-1">{med.instructions}</div>
                      )}
                    </td>
      ...
      {inactiveMedications.length > 0 && (
        <div>
          <h3 className="text-lg font-medium mb-3">{t('patients.medications.inactive')}</h3>
          <div className="bg-base-100 rounded-lg shadow-sm overflow-x-auto">
            <table className="min-w-full divide-y divide-base-300">
              <thead className="bg-base-200">
                <tr>
              ...
              <tbody className="bg-base-100 divide-y divide-base-300">
                {inactiveMedications.map(med => (
                  <tr key={med.id} className="hover:bg-base-200 text-neutral">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                      <div>{formatDate(med.startDate)} - {formatDate(med.endDate)}</div>
                      {med.instructions && (
                        <div className="text-neutral text-xs mt-1">{med.instructions}</div>
                      )}
                    </td>
      ...
      {medications.length === 0 && !showForm && (
        <div className="text-center py-8 bg-base-100 rounded-lg shadow-sm">
          <p className="text-neutral mb-4">{t('patients.medications.noMedications')}</p>
          <button
