    ...
    try {
      ...
      if (response.data.success) {
        ...
        setEconomyData((prevData) => {
          ...
          const monthIndex = updatedMonthlyData.findIndex(
          ...
          // Recalculate totals
          const actualIncomeTotal = updatedMonthlyData.reduce(
            (sum, month) => sum + month.actualIncome,
            0
          );
          const budgetTotal = updatedMonthlyData.reduce(
            (sum, month) => sum + month.budget,
            0
          );
          const predictedIncomeTotal = updatedMonthlyData.reduce(
            (sum, month) => sum + month.predictedIncome,
            0
          );
          ...
          <table className="min-w-full divide-y divide-base-300">
            <thead className="bg-base-200">
              <tr>
                <th
                  scope="col"
                  className="px-6 py-3 text-left text-xs font-medium text-neutral uppercase tracking-wider"
                >
                ...
                {canEdit && (
                  <th
                    scope="col"
                    className="px-6 py-3 text-right text-xs font-medium text-neutral uppercase tracking-wider"
                  >
              ...
              <tr>
                ...
                <td className="px-6 py-3 whitespace-nowrap font-bold">
                  {new Intl.NumberFormat("sv-SE", {
                    style: "currency",
                    currency: "SEK",
                  }).format(economyData.totals.actualIncome)}
                </td>
                <td className="px-6 py-3 whitespace-nowrap font-bold">
                  {new Intl.NumberFormat("sv-SE", {
                    style: "currency",
                    currency: "SEK",
                  }).format(economyData.totals.budget)}
                </td>
                <td className="px-6 py-3 whitespace-nowrap font-bold">
                  {new Intl.NumberFormat("sv-SE", {
                    style: "currency",
                    currency: "SEK",
                  }).format(economyData.totals.predictedIncome)}
                </td>
                {canEdit && <td></td>}
              </tr>
            </tfoot>
