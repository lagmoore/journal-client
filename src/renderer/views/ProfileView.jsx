  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || t('profile.errors.passwordChangeFailed'));
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-base-100 rounded-lg shadow-card border border-base-300 overflow-hidden">
          <div className="bg-base-200 px-4 py-3 border-b border-base-300">
            <h2 className="text-lg font-medium">{t('profile.sections.info')}</h2>
          </div>
        ...
        <div className="bg-base-100 rounded-lg shadow-card border border-base-300 overflow-hidden">
          <div className="bg-base-200 px-4 py-3 border-b border-base-300">
            <h2 className="text-lg font-medium">{t('profile.sections.password')}</h2>
          </div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-3 pr-10 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary ${
                      passwordErrors.currentPassword ? 'border-error' : 'border-base-300'
                    }`}
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handlePasswordChange}
                    className={`w-full pl-3 pr-10 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary ${
                      passwordErrors.newPassword ? 'border-error' : 'border-base-300'
                    }`}
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary ${
                    passwordErrors.passwordConfirmation ? 'border-error' : 'border-base-300'
                  }`}
                  placeholder={t('profile.placeholders.passwordConfirmation')}
                />
                {passwordErrors.passwordConfirmation && (
                  <p className="text-error text-sm mt-1">{passwordErrors.passwordConfirmation}</p>
                )}
              </div>
        ...
        <div className="bg-base-100 rounded-lg shadow-card border border-base-300 overflow-hidden">
          <div className="bg-base-200 px-4 py-3 border-b border-base-300">
            <h2 className="text-lg font-medium">{t('profile.sections.account')}</h2>
          </div>
          
          <div className="p-6">
            <div className="space-y-4">
              <div>
                <div className="text-sm font-medium text-neutral mb-1">{t('users.fields.role')}</div>
                <div className="font-medium">{userData?.role ? t(`users.roles.${userData.role}`) : '-'}</div>
              </div>
              
              <div>
                <div className="text-sm font-medium text-neutral mb-1">{t('users.fields.lastLogin')}</div>
                <div>{userData?.lastLogin ? new Date(userData.lastLogin).toLocaleString() : t('common.never')}</div>
              </div>
              
              <div>
                <div className="text-sm font-medium text-neutral mb-1">{t('users.fields.createdAt')}</div>
                <div>{userData?.createdAt ? new Date(userData.createdAt).toLocaleString() : '-'}</div>
              </div>
            </div>
          </div>
        </div>
