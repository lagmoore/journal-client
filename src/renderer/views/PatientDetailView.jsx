...
const PatientDetailView = () => {
    ...
    const fetchPatientData = async () => {
      ...
      try {
        ...
        if (patientResponse.data.success) {
          ...
          // Fetch journals
          const journalsResponse = await api.get(`/patients/${patientId}/journals`, {
            headers: { Authorization: `Bearer ${accessToken}` }
          });
  ...
  const handleCreateJournal = (entryType = 'note', medication = null) => {
    ...
    navigate(`/dashboard/journals/new?patientId=${patientId}&entryType=${entryType}`, { 
      state: { initialData } 
    });
  };
  ...
  const handleCreateMedication = async (medicationData) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || t('patients.medications.createError'));
        return false;
      }
    } catch (error) {
  ...
  const handleUpdateMedication = async (medicationId, medicationData) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || t('patients.medications.updateError'));
        return false;
      }
    } catch (error) {
  ...
  const handleDeleteMedication = async (medicationId) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || t('patients.medications.deleteError'));
        return false;
      }
    } catch (error) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        {!isNewPatient && (
          <div className="flex gap-2">
            ...
            {patient.isActive ? (
              <button
                onClick={() => handleSavePatient({ ...patient, isActive: false })}
              ...
              <button
                onClick={() => handleSavePatient({ ...patient, isActive: true })}
      ...
      <ConfirmDialog
        ...
        message={t('patients.deleteDialog.message', { name: `${patient?.firstName} ${patient?.lastName}` })}
