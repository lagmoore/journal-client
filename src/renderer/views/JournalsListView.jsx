...
const JournalsListView = () => {
  ...
  const filteredJournals = journals.filter(journal => {
    ...
    if (searchTerm) {
      ...
      if (
        ...
        !(journal.category && journal.category.toLowerCase().includes(lowercaseSearch))
      ) {
        return false;
      }
    }
  ...
  const sortedJournals = [...filteredJournals].sort((a, b) => {
    ...
    } else if (filters.sortBy === 'patientAZ' || filters.sortBy === 'patientZA') {
  ...
  const categories = [...new Set(journals.filter(j => j.category).map(j => j.category))];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          ...
          <div>
            ...
            >
              ...
              <option value="patientAZ">{t('journals.filters.patientAZ')}</option>
              <option value="patientZA">{t('journals.filters.patientZA')}</option>
            </select>
          </div>
